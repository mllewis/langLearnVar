...
\end{center}
\vspace{-.5em}
\caption{FIX THISPressures on language internal and external to the cognitive system, at three different timescales. Brackets show examples of phenomena that result from a particular pressure at a particular timescale. The Linguistic Niche Hypothesis suggests that language evolution is influenced by the internal and external pressures in the particular environmental context in which a language is spoken.}
...
\subsection{Method}
Datasets were merged using common ISO-639 codes. Five variables were log-transformed to better approximate a normal distribution (population, L2 to L1 ratio,  number of neighbors, area, number of consonants, number of vowels).\footnote{All code and data for the paper are available at \url{http://github.com/mllewis/langLearnVar}} 
...
This finding is broadly consistent with previous work that finds relationships between  individual metrics of complexity and various measures of population size \cite<e.g.,>{lupyan2010language}. While our results are broadly consistent with the existing literature, we find null effects for several of the relationships reported in the literature. In some cases, this may be due to HERE.
...
In Study 2, we explore this prediction. As a proxy for learnability for L1 learners, we use the mean age of acquisition (AoA) of words in a language. We predict that languages that  are more easily learnable by L1 speakers should have earlier AoAs than languages with a large L2 pressure.
