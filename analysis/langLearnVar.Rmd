 ...
 * Bentz, et al. (2015): L2 learners and lexical diversity
    + number of first language speakers (n.L1.speakers)  - sent in a separate email
    + number of second language speakers (n.L2.speakers)  - sent in a separate email
    + ratio of L2 to L1 (ratio.L2.L1) - sent in a separate email
 ...
 * Bentz, et al. (2016): Entropy measures
    +  Entropy calculated from universal declaration of human rights (H_scaled.UDHR)
    +  Entropy cacluclate from parallel bible corpus (H_scaled.PBC)
    +  Entropy cacluclated from European Parliament Corpus (H_scaled.EPC)
 ...
 * Lewis & Frank (2016): Complexity bias
 ...
 * Pellegrino, Coupe, & Marsico (2011): Information density
     + information.density
     + syllable.rate
     + information.rate
...
TO DO:
* Add in new complexity norms
* add in futrell dependency lengths (need languaeg key)
* add in other futrell data
* re-run all analyses (particularly PCA) 
* look at new data source Gary pointed to
...
ld.c$adjusted.complexity = ld.c$morphological.complexity.ld / ld.c$propPresent
```

Bentz, et al. (2015): Lexical diversity. Note here that we are only looking at languages from the UDHR corpus. The other corpora have smaller languages only.
```{r}
bentz_2015 = read.csv("../data/bentz_2015.csv") %>%
  gather(temp, LDT, starts_with("LDT")) %>% 
  unite(temp1, measure, temp, sep = ".") %>% 
  spread(temp1, LDT) %>%
  filter(text == "UDHR") %>%
  select(iso_639_3, a.LDTscaled, 
         H.LDTscaled, TTR.LDTscaled, TTR.LDT) %>%
...
Bentz, et al. (2015): L2 speakers
```{r}
bentz_2015_l2= read.csv("../data/bentz_2015_L2.csv") %>%
  select(iso, l1, l2, ratio) %>%
  rename(ISO = iso,
         n.L1.speakers = l1,
         n.L2.speakers = l2,
         ratio.L2.L1 = ratio) 
...
Bentz, et al. (2016): Entropy measures. Some languages have multiple values for a corpus. Why is this? Just taking first value for now...
```{r}
bentz_2016= read.csv("../data/bentz_2016.csv") %>%
  select(iso, Corpus, H_scaled) %>%
  group_by(iso, Corpus) %>%
  slice(1) %>%
  spread(Corpus, H_scaled) %>%
  rename(H_scaled.EPC =  EPC,
         H_scaled.PBC = PBC,
         H_scaled.UDHR = UDHR, 
         ISO = iso)
```
...
d = full_join(ld.demo.qual, cb, by = "ISO") %>%
  ...
  full_join(bentz_2015, by = "ISO") %>%
  full_join(bentz_2015_l2, by = "ISO") %>%
  full_join(bentz_2016, by = "ISO") %>%
...
d.scatter.fam = d.clean %>% 
  select(pop_log, ratio.L2.L1_log,
         ...
         sd.precip_log,adjusted.complexity, TTR.LDT, p.complexity.bias,
         mean.length, n.consonants_log, n.vowels_log,
         lang.family, native.country, adjusted.complexity) %>%
  gather(lang_measure, lang_value,
...
Temperature is positively correlated with AOA, and precipitation variability is negatively correlated. Children learn words later in hotter places with little variability in preicipitation.
...
#### AOA and PCA
```{r}
tidy(lmer(mean.aoa ~ PC1_L + (PC1_L|lang.family) +
                 (1|native.country), data=all.pca,
          control=lmerControl(optimizer = "bobyqa")))

tidy(lmer(mean.aoa ~ PC2_L + (PC2_L|lang.family) +
                 (1|native.country), data=all.pca,
          control=lmerControl(optimizer = "bobyqa")))

tidy(lmer(mean.aoa ~ small + (small|lang.family) +
                 (1|native.country), data=all.pca,
          control=lmerControl(optimizer = "bobyqa")))

tidy(lmer(mean.aoa ~ hot + (hot|lang.family) +
                 (1|native.country), data=all.pca,
          control=lmerControl(optimizer = "bobyqa")))
```
AoA not reliably predicted by any of the four PCs.
