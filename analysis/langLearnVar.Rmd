...
d.scatter.fam = d.clean %>% 
  ...
  #mutate_each(funs(as.numeric(scale(.))), c(-15,-16)) %>% #scaling doesn't change anything
  gather(lang_measure, lang_value,
...
ggplot(d.scatter.fam, aes(x = pop_value, y = lang_value)) +
  ...
  geom_point(size = .3) +
  geom_abline(line.fits, mapping = aes(slope = s, intercept = i), size = 2, color = "green") +
  facet_grid(lang_measure ~ pop_measure, scales = "free", 
             labeller = labeller(pop_measure = facet_pop_names, lang_measure = facet_lang_names)) +
  scale_fill_manual(values = c( "mediumblue", "grey99", "red1")) +
  theme_bw() +
  xlab("\nEnvironmental variables") +
  ylab("Language variables\n") +
  theme(legend.position = "none", axis.title = element_text(size = 21), strip.text= element_text(size = 17))
dev.off()
```
...
# TTR 
tidy(lmer(mean.aoa ~ TTR.LDT+ (TTR.LDT|lang.family) + (1|native.country), d.clean))[2, "statistic"]
