...
# were getting country here from choosing the most frequent country for a language
fams.country = ld %>%
        group_by(ISO) %>%
...
# do pca
pca = prcomp(na.omit(d.demo), scale = TRUE)
...
# plot loadings
pcs <- cbind(names(d.demo),
                    gather(as.data.frame(pca$rotation), 
                           pc, value))
names(pcs)[1] = "variable"
 
pcs$variable =  factor(pcs$variable , levels = pcs$variable[1:8]) # order variables
pcs$magnitude = ifelse(abs(pcs$value)>.2, TRUE, FALSE)
ggplot(pcs) +
...
d.pca = d.clean[good_is,] %>%
  cbind(pca$x)

names(d.pca)[names(d.pca) == "PC1"] = "wet_and_hot_climate"
names(d.pca)[names(d.pca) == "PC2"] = "small"
names(d.pca)[names(d.pca) == "PC3"] = "desert_climate"
```
...
geom_point(aes(x = lon, y=lat, 
               color = wet_and_hot_climate), size=3) +
...
demo_vars_crit2 = c("wet_and_hot_climate", "small", "desert_climate")
...
d.scatter.fam = d.pca %>% 
  select(n.consonants_log, n.vowels_log, 
         ...
         lang.family, country, wet_and_hot_climate, small,
         desert_climate) %>%
  gather(lang_measure, lang_value, 
...
d.model.fits = d.scatter.fam %>%
  group_by(lang_measure, pop_measure) %>%
  do(tidy(lmer(lang_value ~ pop_value + (pop_value|lang.family) + (1|country), data=.))) %>%
  filter(term == "pop_value") %>%
  mutate(sig.col = ifelse(statistic > 1.96, "pos",
                          ifelse(statistic < -1.96, "neg",
                                 "none"))) %>%
...
```{r}
ld.demo.qual %>%
  mutate(morphological.complexity.factor = as.factor(abs(morphological.complexity))) %>%
  ...
  mutate(morphological.complexity = as.numeric(as.character(morphological.complexity.factor))) %>%
  ggplot() +
