...
WALS features of complexity - Lupyan and Dale data [1477,but only 864 with ISOs, lang names of missing ones don't match other ld data set e.g. "zaporo", I think these are the ones with no data?]
There are 24 variables used total (not clear what those are)
variables with missing data included, which means a count reflects a zero for missing data -- can't use this
```{r, eval = F}
...
ld.c = read.table("../data/lupyan_2010_complexity.txt", fill = T, 
  ...
  select(morphological.complexity.ld, morphological.complexity.ld.centered, ISO, lang)

ld.c$adjusted.complexity = ld.c$morphological.complexity.ld * ld.c$propPresent

d.clean = d.clean %>%
            left_join(ld.c, by = "ISO")
...
m = ld.c %>%
  ...
  group_by(missing) %>%
  summarize(complexity = mean(complexity))
```
